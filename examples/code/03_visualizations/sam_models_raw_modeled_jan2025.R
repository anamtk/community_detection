...
#bird datasets:
bird_q_sam <- readRDS(here('02_konza_birds',
                           'data_outputs',
                           'SAM',
                           'model_outputs',
                           'knz_q_SAM_summary.RDS'))

bird_q_sam_raw <- readRDS(here('02_konza_birds',
                           'data_outputs',
                           'SAM',
                           'model_outputs',
                           'knz_SAM_Q_summary_raw.RDS'))

bird_fric_sam <- readRDS(here('02_konza_birds',
                           'data_outputs',
                           'SAM',
                           'model_outputs',
                           'knz_fric_SAM_summary.RDS'))

bird_fric_sam_raw <- readRDS(here('02_konza_birds',
                               'data_outputs',
                               'SAM',
                               'model_outputs',
                               'knz_SAM_fric_summary_raw.RDS'))
...
#plant datasets:
plant_gain_sam <- readRDS(here('04_nps_plants',
                               'data_outputs',
                               'SAM',
                               'model_outputs',
                               'nps_gain_SAM_summary.RDS'))

plant_gain_sam_raw <- readRDS(here('04_nps_plants',
                               'data_outputs',
                               'SAM',
                               'model_outputs',
                               'nps_SAM_gains_summary_raw.RDS'))

plant_loss_sam <- readRDS(here('04_nps_plants',
                               'data_outputs',
                               'SAM',
                               'model_outputs',
                               'nps_losses_SAM_summary.RDS'))

plant_gain_sam <- readRDS(here('04_nps_plants',
                               'data_outputs',
                               'SAM',
                               'model_outputs',
                               'nps_SAM_losses_summary_raw.RDS'))
...
bird_q_plot <- sam_plot_fun(model_model = bird_q_sam, model_raw = bird_q_sam_raw) +
  scale_y_discrete(labels = c("Temperature", "Precipitation")) + 
  labs(title = "Bird Rao")

bird_fric_plot <- sam_plot_fun(model_model = bird_fric_sam, model_raw = bird_fric_sam_raw) +
  scale_y_discrete(labels = c("Temperature", "Precipitation")) + 
  labs(title = "Bird FRic")
