model{
  ...
  #This model also includes a covariance matrix among bird abundances, such
  ...
  ## 4. Covariates on detection including bird size and survey length
    ...
    for(i in 1:n.transects){ #loop through each transect
      ...
      for(r in 1:n.rep[i,t]){ #for the number of surveys on each transect in each year
        ...
        logit(p[k,i,t,r]) <- a0[k] + #species-level intercept
          a1.Effort*effort[i,t,r] + #effort effect, not species dependent
