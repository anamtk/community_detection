...
mod <- readRDS(file ="/scratch/sml665/nps_plants/outputs_no_lifegroup/nps_MSAM_model3.RDS")
...
data <- readRDS("/scratch/sml665/nps_plants/inputs_no_lifegroup/nps_msam_multisite_subset.RDS")
...
inits <- list(list(N = data$z,
              ...
              list(N = data$z,
                   mu.lpsi = samp_df2$mu.lpsi + 0.03,
                   sig.lpsi = samp_df2$sig.lpsi + 0.03,
                   mu.a0 = samp_df2$mu.a0 + 0.2,
                   sig.a0 = samp_df2$sig.a0 + 0.03,
                   a1.Cover = samp_df2$a1.Cover + 0.02,
                   mu.missingcover = samp_df2$mu.missingcover + 0.002,
                   sig.missingcover = samp_df2$sig.missingcover + 0.002),
              list(N = data$z,
                   mu.lpsi = samp_df2$mu.lpsi - 0.03,
...
mod2 <- jagsUI::jags(data = data_list,
                    inits = inits,
                    #inits = NULL,
                    model.file = '/scratch/sml665/nps_plants/inputs_no_lifegroup/nps_MSOM_simple_SL.R',
...
saveRDS(mod2, 
        file ="/scratch/sml665/nps_plants/outputs_no_lifegroup/nps_MSAM_model4.RDS")
...
mcmcplot(mod2$samples,
         dir = "/scratch/sml665/nps_plants/outputs_no_lifegroup/mcmcplots/MSAM4")

# Get RHat per parameter ------------------------------------------------

Rhat <- mod2$Rhat

saveRDS(Rhat, "/scratch/sml665/nps_plants/outputs_no_lifegroup/nps_MSAM_model_Rhat4.RDS")
